...
- [ ] Create `crates/robotcode-rf-parser/`
- [ ] **Lexer** (`lexer/`):
  - [ ] Token type enum mirroring `robot.parsing.lexer.tokens.Token`
  - [ ] Logos-based scanner for RF's context-sensitive tokenization (indent-sensitive, header keywords, etc.)
  - [ ] Lexer modes: FILE, SETTING_HEADER, TEST_CASE_HEADER, KEYWORD_HEADER, VARIABLE_HEADER, TESTCASE_BODY, KEYWORD_BODY, etc.
  - [ ] Error token handling (non-crashing on malformed input)
- [ ] **Parser** (`parser/`):
  - [ ] Complete AST node hierarchy (see below)
  - [ ] Recursive-descent parser producing typed AST from token stream
  - [ ] Error recovery: skip to next logical boundary on syntax error
  - [ ] Preserve all source position information (line, column, end_line, end_column)
  - [ ] Trivia preservation (comments, whitespace) for formatter use
- [ ] **AST Node Types** (mirror `robot.parsing.model.*`):
  - [ ] `File`, `SettingSection`, `VariableSection`, `TestCaseSection`, `KeywordSection`, `CommentSection`
  - [ ] Settings: `LibraryImport`, `ResourceImport`, `VariablesImport`, `Documentation`, `Tags`, `Suite Setup/Teardown`, `Test Setup/Teardown`, `Test Template`, `Force/Default Tags`, `Metadata`
  - [ ] `Variable` (variable declaration)
  - [ ] `TestCase`, `Keyword` (block nodes)
  - [ ] Statements: `KeywordCall`, `Arguments`, `ReturnStatement`, `IfHeader`/`ElseIfHeader`/`ElseHeader`/`EndHeader`, `ForHeader`/`WhileHeader`/`TryHeader`/`ExceptHeader`/`FinallyHeader`, `BreakStatement`, `ContinueStatement`, `TemplateArguments`, `Comment`, `EmptyLine`
- [ ] **Variable Utilities** (`variables.rs`):
  - [ ] Port `robot.variables.search` — `is_variable`, `search_variable`, `contains_variable`, `is_scalar_assign`
  - [ ] Variable types: scalar `${x}`, list `@{x}`, dict `&{x}`, env `%{x}`
- [ ] **Escaping** (`escaping.rs`): Port `robot.utils.escaping.unescape` and `split_from_equals`
- [ ] **Multi-version support** (`versions.rs`): RF 5.x / 6.x / 7.x syntax differences
- [ ] **Visitor trait** (`visitor.rs`): Generic `AstVisitor` trait with default no-op implementations
- [ ] **Snapshot tests**: Parse all `.robot` test fixtures from `tests/` directory; compare AST output against expected snapshots
...
|-------|------|----------|------------|
| 1 | Foundation | 4–6 weeks | Cargo workspace, core crate |
| 2 | RF Parser | 8–10 weeks | Rust `.robot` parser |
...
- Rust parser handles all test fixtures
- Benchmark shows ≥10× improvement over Python parser
